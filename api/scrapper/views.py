...
def dashboard_view(request):
    ...
    recent_scraped_news = NewsPost.objects.filter(source_type="scraped").select_related(
        "author"
    )[:5]
...
def sources_list_view(request):
    ...
    return render(request, "scraper/sources_list.html", {"sources_page": sources_page})
...
def source_create_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_manual:
                ...
                messages.success(
                    request, "Fonte criada e notícia publicada manualmente com sucesso!"
                )
            else:
                messages.success(request, "Fonte de scraping criada com sucesso!")
    ...
    return render(
        request,
        "scraper/source_form.html",
        {"form": form, "title": "Nova Fonte de Scraping"},
    )
...
def source_edit_view(request, source_id):
    ...
    return render(
        request,
        "scraper/source_form.html",
        {"form": form, "title": "Editar Fonte", "source": source},
    )
...
def run_scraping_view(request, source_id):
    ...
    if is_manual:
        try:
            ...
            messages.error(request, f"Erro na publicação manual: {str(e)}")
    ...
    if result.get("success"):
        ...
        messages.error(request, f"Erro no scraping: {result.get('error')}")
...
def jobs_history_view(request):
    jobs = ScraperJob.objects.select_related("source", "started_by").order_by(
        "-created_at"
    )
    ...
    return render(
        request,
        "scraper/jobs_history.html",
        {"jobs_page": jobs_page, "status_filter": status_filter},
    )
...
def news_edit_view(request, news_id):
    ...
    return render(request, "scraper/news_edit.html", {"form": form, "news": news})

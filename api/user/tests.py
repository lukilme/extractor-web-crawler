...
class AuthenticationViewsTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            username="viewuser", password="testpass123", user_type="comum"
        )
        self.admin_user = User.objects.create_user(
            username="adminview", password="adminpass123", user_type="admin"
        )
        ...
        with open(tmp_file.name, "rb") as img:
            response = self.client.post(
                reverse("accounts:profile_edit"),
                {
                    ...
                    "bio": "Usuário com avatar",
                    "avatar": SimpleUploadedFile(
                        "test_avatar.jpg", img.read(), content_type="image/jpeg"
                    ),
                },
            )
